...
def _make_vehicle(
    ...
    return VehicleState(
        ...
        soc_source_type=soc_source_type,
...
class TestFullPipeline:
    ...
    def test_single_vehicle_normal_charging(self):
        ...
        ctx = _make_context(
            [v], grid_power_w=5000, power_limit_kw=11.0, current_export_price=0.10, night_price=0.80
        )
        with patch(_UTCNOW_PATH, return_value=_FIXED_NOW):
    ...
    def test_single_vehicle_export_profitable(self):
        ...
        v = _make_vehicle("car_1", current_soc=50)
        ctx = _make_context(
            [v], grid_power_w=3000, power_limit_kw=11.0, current_export_price=2.00, night_price=0.20
        )
        with patch(_UTCNOW_PATH, return_value=_FIXED_NOW):
            result = run_decision_pipeline(ctx)
        assert result.vehicles[0].allocated_amps == 0
    ...
    def test_data_output_completeness(self):
        ...
        v = _make_vehicle("car_1", current_soc=50)
        ctx = _make_context(
            [v], grid_power_w=5000, power_limit_kw=11.0, current_export_price=0.50, night_price=0.30
        )
        with patch(_UTCNOW_PATH, return_value=_FIXED_NOW):
            result = run_decision_pipeline(ctx)
