...
def test_vehicle_state_needs_charge():
    state = VehicleState(
        vehicle_id="car_1", name="Family Car", priority=1, target_soc=80,
        current_soc=50, departure_time=None, is_connected=True,
        soc_source_type=SOC_SOURCE_API, soc_entity_id="sensor.car_soc",
        charger_entity_id="sensor.easee_status", allocated_amps=0, allocated_phases=1,
    )
...
def test_vehicle_state_no_charge_needed():
    state = VehicleState(
        vehicle_id="car_1", name="Family Car", priority=1, target_soc=80,
        current_soc=85, departure_time=None, is_connected=True,
        soc_source_type=SOC_SOURCE_API, soc_entity_id="sensor.car_soc",
        charger_entity_id="sensor.easee_status", allocated_amps=0, allocated_phases=1,
    )
...
def test_vehicle_state_needs_charge_soc_unknown():
    state = VehicleState(
        vehicle_id="car_1", name="Family Car", priority=1, target_soc=80,
        current_soc=None, departure_time=None, is_connected=True,
        soc_source_type=SOC_SOURCE_NONE, soc_entity_id=None,
        charger_entity_id="sensor.easee_status", allocated_amps=0, allocated_phases=1,
    )
    assert state.needs_charge is True
...
def test_vehicle_state_disconnected_no_charge():
    state = VehicleState(
        vehicle_id="car_1", name="Family Car", priority=1, target_soc=80,
        current_soc=50, departure_time=None, is_connected=False,
        soc_source_type=SOC_SOURCE_API, soc_entity_id="sensor.car_soc",
        charger_entity_id="sensor.easee_status", allocated_amps=0, allocated_phases=1,
    )
    assert state.needs_charge is False
...
def test_build_vehicle_state():
    cfg = VehicleConfig(
        ...
        charger_entity_id="sensor.easee_status", soc_entity_id="sensor.car_soc",
